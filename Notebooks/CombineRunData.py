...
parser.add_argument('--version', type=str, help='Dataset version to work with.')
...
for ich in range(len(channels)):
    ...
    # np.save(Outpath + f"/{channel}_AllSignals.npy", s)
    # np.save(Outpath + f"/{channel}_AllNoiseOnly.npy", n)
    # np.save(Outpath + f"/{channel}_AllSigPlusNoise.npy", ns)
